...
leaps <- regsubsets(TiO2 ~ TK + Pkb + H2O + TimeH + M + FM + InvTK, data = CationFractionsTitanite, nbest = 2)
plot(leaps, scale = "adjr2")
# summary(leaps)
```

Since TK and InvTK convey the same information, and theory suggests that titanite solubility should depend on inverse temperature, we include only InvTK in the analysis. Results of subsets regression suggest that M, InvTK, Pkb, and TimeH are the significant independent variables.
...
"Global stat" and "Link Function" assumptions not satisfied [@pena2006global]
...
RegModel <- RegModel %>%
  mutate(
    TiO2sat = (out$coefficients[2,1] * M + out$coefficients[3,1] * InvTK + out$coefficients[1,1]),
...
```{r}
TiO2ModelTable <- RegModel %>%
  transmute(
    Expt,
    M = round(M, digits = 2),
    InvTK = round(1000*InvTK, digits = 3),
    TiO2,
    TiO2sat = round(TiO2sat, digits = 2),
    TiO2satSD = round(TiO2satSD, digits = 2)
  )
kable(TiO2ModelTable, format = "html",
             col.names = c("Expt.", "M", "1000*1/T(K)", "TiO~2~", "TiO~2,sat~", "1$\\sigma$"))
