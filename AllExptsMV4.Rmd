...
This program imports the file AllExptsV3.csv containing silicate glass compositions from: 1. titanite growth experiments conducted at Vanderbilt University by graduate student Danny Flanagan. 2. titanite dissolution experiments conducted at RPI by Rick Ryerson and Bruce Watson. 3. phase equilibrium experiments from the LEPR database [@Hirschmann2008] that produced titanite. Thermodynamics suggests that a titanite solubility equation should have the form:
...
write.csv(CationFractionsTitanite, file = "TiO2AllExptsM_Table.csv")
...
The numbers labeling the outliers correspond to the row numbers of the outlier experiments. At each step I delete all three, refit the model, and then use the plots to identify the next set of three outliers.

```{r}
DelExpts <- c("SpD_1", "SpD_3", "SpG_28")
...
RegModel <- RegModel %>%
  mutate(
   TiO2sat = (Mcoef * M + Tcoef * TK + Int),
    TiO2satSD = TiO2sat*sqrt(Mcoef^2*Mcoefsd^2 + Tcoef^2*Tcoefsd^2),
...
```{r}
safe_colorblind_palette <- c("#88CCEE", "#CC6677", "#DDCC77", "#117733", "#332288", "#AA4499", "#44AA99", "#999933", "#882255", "#661100", "#6699CC", "#888888", "#0072B2", "#D55E00")
...
p1 <- ggMarginal(p, type = "histogram")
print(p1)
ggsave("Fig5.eps", width = 2000, height = 2*618, units = "px", plot = p1)
```
...
print(p)
ggsave("Fig7.png", width = 2000, height = 2*618, units = "px", plot = p)
...
```{r}
p <- ggplot(RegModel, aes(x=TC, y=TiO2, shape = Source, color = Source)) +
  ...
  xlab("Temperature (°C)") +
   theme_bw()
print(p)
ggsave("Fig6a.eps", width = 2000, height = 2*618, units = "px", plot = p)
```

### TiO2-M plot

```{r}
p <- ggplot(RegModel, aes(x=M, y=TiO2, shape = Source, color = Source)) +
  geom_point(size = 2, stroke = 1) +
  scale_shape_manual(values=c(0, 1, 2)) +
  scale_color_manual(values=safe_colorblind_palette) +
  ylab(~paste("TiO"[2]," (wt. %)")) +
  xlab("M") +
   theme_bw()
print(p)
ggsave("Fig6b.eps", width = 2000, height = 2*618, units = "px", plot = p)
```
...
```{r}
TiO2AllExptsModelTable <- select(RegModel, Source, Expt, TiO2, CaO, M, FM, A_CNK, A_NK, A_C, TK, TiO2sat, TiO2satSD)
write.csv(TiO2AllExptsModelTable, file = "TiO2AllExptsM_ModelTable.csv")
DFGlassComps <- filter(RegModel, Source != "LEPR") %>%
  select(Expt, SiO2:TiO2, M, FM, A_CNK, A_NK, A_C)
